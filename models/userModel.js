...
var createUser = (user, callback) => {
    ...
    db.executeQuery(sql, [user.name, user.phone, user.email, 0, user.password, user.address, user.gender, user.image], function(result) {
        callback(result);
    });
};
...
var updateUser = (user, callback) => {
    var sql = "UPDATE users SET name = ?, email = ?, phone = ?, address = ?, gender = ?, image = ? WHERE user_id = ?";
    db.executeQuery(sql, [user.name, user.email, user.phone, user.address, user.gender, user.image, user.user_id], function(result) {
        callback(result);
    });
};
